...
async def manage_channels(callback: CallbackQuery):
    ...
    await callback.message.edit_text(channels_text, reply_markup=AdminKeyboard.manage_channels_menu())
...
async def process_channel_name(message: Message, state: FSMContext):
    ...
    await message.answer(f"Введите ID канала для '{channel_name}' (должно быть целое число, например: -1001234567890):")
...
async def process_channel_id(message: Message, state: FSMContext):
    ...
    await message.answer("Введите ссылку-приглашение для канала (должна начинаться с https://t.me/ или t.me/):")
...
async def process_channel_link(message: Message, state: FSMContext):
    ...
    channel = await ChannelDAL.create_channel(name=channel_name, channel_id=channel_id, invite_link=channel_link)
    ...
    for plan in tariff_plans:
        builder.add(
            InlineKeyboardButton(
                text=f"{plan.name} ({plan.price}₽)", callback_data=f"channel:add_plan:{channel.id}:{plan.id}"
            )
        )

    builder.add(InlineKeyboardButton(text="◀️ Назад к управлению каналами", callback_data="admin:manage_channels"))

    builder.adjust(1)

    await message.answer("Выберите тарифные планы для канала:", reply_markup=builder.as_markup())
...
async def remove_plan_from_channel(callback: CallbackQuery):
    ...
    if has_active_subscriptions:
        await callback.answer(
            "Невозможно отвязать тариф от канала, так как существуют активные подписки на него.", show_alert=True
        )
        return
...
async def list_channels_for_edit(callback: CallbackQuery):
    ...
    if not channels:
        await callback.message.edit_text(
            "📋 Каналы не найдены. Сначала добавьте каналы.", reply_markup=AdminKeyboard.manage_channels_menu()
        )
        await callback.answer()
        return
    ...
    for channel in channels:
        ...
        builder.add(InlineKeyboardButton(text=f"{status} {channel.name}", callback_data=f"channel:edit:{channel.id}"))

    builder.add(InlineKeyboardButton(text="◀️ Назад", callback_data="admin:manage_channels"))

    builder.adjust(1)

    await callback.message.edit_text("📝 Выберите канал для редактирования:", reply_markup=builder.as_markup())
    await callback.answer()
...
async def edit_channel(callback: CallbackQuery, state: FSMContext):
    ...
    builder.add(
        ...
        InlineKeyboardButton(
            text=f"Активность: {'Вкл ✅' if channel.is_active else 'Выкл ❌'}", callback_data="channel:field:active"
        ),
    ...
    await callback.message.edit_text(channel_info, reply_markup=builder.as_markup(), parse_mode="HTML")
    await callback.answer()
...
async def edit_channel_field(callback: CallbackQuery, state: FSMContext):
    ...
    if field == "active":
        ...
        await callback.answer(f"Канал {'активирован' if channel.is_active else 'деактивирован'}", show_alert=True)
    ...
    field_names = {"name": "название", "link": "ссылку-приглашение"}
...
async def process_channel_new_value(message: Message, state: FSMContext):
    ...
    for ch in channels:
        ...
        builder.add(InlineKeyboardButton(text=f"{status} {ch.name}", callback_data=f"channel:edit:{ch.id}"))
    ...
    await message.answer(
        "📝 Выберите канал для редактирования или вернитесь к управлению каналами:", reply_markup=builder.as_markup()
    )
...
async def edit_channel_plans(callback: CallbackQuery):
    ...
    builder.add(
        InlineKeyboardButton(text="◀️ Назад к редактированию канала", callback_data=f"channel:edit:{channel_id}")
    )
...
async def delete_channel(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📝 Управление каналами", reply_markup=AdminKeyboard.manage_channels_menu())
