...
class ChannelDAL:
    ...
    async def get_active_channels() -> List[Channel]:
        ...
        """
        query = select(Channel).where(Channel.is_active == True).order_by(Channel.display_order)
    ...
    async def toggle_active(channel_id: int) -> Optional[Channel]:
        ...
        query = update(Channel).where(Channel.id == channel_id).values(is_active=new_state).returning(Channel)
        ...
        for channel_data in default_channels:
            ...
            if channel is None:
                ...
                if need_update:
                    ...
                    query = update(Channel).where(Channel.id == channel.id).values(**update_data).returning(Channel)
    ...
    async def create_channel(
        name: str, channel_id: int, invite_link: str, display_order: Optional[int] = None
    ) -> Channel:
        """
        Создать новый канал
        ...
        async with ChannelDAL.db.session() as session:
            channel = Channel(
                name=name, channel_id=channel_id, invite_link=invite_link, is_active=True, display_order=display_order
            )
            session.add(channel)
            await session.commit()
            await session.refresh(channel)
    ...
    @staticmethod
    async def update_channel(channel_id: int, **kwargs) -> Optional[Channel]:
        """
        Обновить канал
        ...
        query = update(Channel).where(Channel.id == channel_id).values(**kwargs).returning(Channel)
    ...
    async def delete_channel(channel_id: int) -> bool:
        ...
        for tariff_id in tariff_ids:
            subs_query = select(Subscription).where(
                and_(Subscription.plan_id == tariff_id, Subscription.is_active == True)
            )
        ...
        if has_active_subscriptions:
            ...
            update_query = update(Channel).where(Channel.id == channel_id).values(is_active=False).returning(Channel.id)
            result = await ChannelDAL.db.fetchval(update_query)
            return result is not None
        else:

            for tariff_id in tariff_ids:
                update_tariff_query = update(TariffPlan).where(TariffPlan.id == tariff_id).values(is_active=False)
    ...
    async def get_channels_with_tariffs() -> List[Tuple[Channel, List[TariffPlan]]]:
        ...
        for channel in channels:
            ...
            tariffs_query = (
                select(TariffPlan)
                .where(and_(TariffPlan.channel_id == channel.id, TariffPlan.is_active == True))
    ...
    async def get_user_available_channels(telegram_user_id: int) -> List[Channel]:
        ...
        query = (
            ...
            .join(User, Subscription.user_id == User.id)
            .where(and_(User.user_id == telegram_user_id, Subscription.is_active == True, Channel.is_active == True))
