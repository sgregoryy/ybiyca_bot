...
class PaymentMethodDAL:
    ...
    async def get_active_methods() -> List[PaymentMethod]:
        ...
        """
        query = select(PaymentMethod).where(PaymentMethod.is_active == True).order_by(PaymentMethod.display_order)
    ...
    async def get_supported_currencies(method_id: int) -> List[Tuple[PaymentMethodCurrency, Currency]]:
        ...
        query = (
            ...
            .where(and_(PaymentMethodCurrency.payment_method_id == method_id, Currency.is_active == True))
        ...
        for method_data in default_methods:
            ...
            for currency_id in supported_currencies:
                await PaymentMethodDAL.add_currency_to_method(
                    method.id, currency_id, is_default=(currency_id == method_data["default_currency_id"])
                )
    ...
    @staticmethod
    async def add_currency_to_method(
        method_id: int, currency_id: int, is_default: bool = False
    ) -> Optional[PaymentMethodCurrency]:
        """
        ...
        query = select(PaymentMethodCurrency).where(
            and_(PaymentMethodCurrency.payment_method_id == method_id, PaymentMethodCurrency.currency_id == currency_id)
        )
        ...
        async with PaymentMethodDAL.db.session() as session:
            method_currency = PaymentMethodCurrency(
                payment_method_id=method_id, currency_id=currency_id, is_default=is_default
            )
        ...
        async with PaymentMethodDAL.db.session() as session:
            ...
            if supported_currency_ids:
                for currency_id in supported_currency_ids:
                    await PaymentMethodDAL.add_currency_to_method(
                        method.id, currency_id, is_default=(currency_id == default_currency_id)
                    )
            else:

                await PaymentMethodDAL.add_currency_to_method(method.id, default_currency_id, is_default=True)

            return method

    @staticmethod
    async def update_method(method_id: int, **kwargs) -> Optional[PaymentMethod]:
        """
        ...
        query = update(PaymentMethod).where(PaymentMethod.id == method_id).values(**kwargs).returning(PaymentMethod)
        ...
        if supported_currency_ids is not None:
            ...
            delete_query = delete(PaymentMethodCurrency).where(PaymentMethodCurrency.payment_method_id == method_id)
            await PaymentMethodDAL.db.execute(delete_query)

            for currency_id in supported_currency_ids:
                await PaymentMethodDAL.add_currency_to_method(
                    method_id, currency_id, is_default=(currency_id == updated_method.default_currency_id)
                )
