...
class TariffPriceDAL:
    ...
    async def get_price_by_code(tariff_id: int, currency_code: str) -> Optional[Tuple[TariffPrice, Currency]]:
        ...
        """
        query = select(TariffPrice, Currency).join(
            Currency, TariffPrice.currency_id == Currency.id
        ).where(
            and_(
                TariffPrice.tariff_id == tariff_id,
    ...
    async def init_default_prices_for_tariff(tariff_id: int, base_price: float) -> List[TariffPrice]:
        ...
        """
        # Базовые курсы конвертации (упрощенный подход)
        exchange_rates = {
            "RUB": 1.0,          # 1 RUB = 1 RUB
            "USD": 0.011,        # 1 RUB ≈ 0.011 USD (примерно 90 RUB за 1 USD)
            "STARS": 10.0,       # 1 RUB ≈ 10 STARS (10 звезд за рубль) 
            "BTC": 0.00000004,   # Очень упрощенно
            "TON": 0.004,        # Примерный курс
            "USDT": 0.011        # Аналогично USD
        }
